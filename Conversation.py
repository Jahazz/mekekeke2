...
class Conversation:
    def __init__(self, conversation_timeout, port):
        ...
        self.port = port
        driver1 = pool.apply_async(self.initialize_client)
        driver2 = pool.apply_async(self.initialize_client)
        self.conversation_timeout = conversation_timeout
        self.id = uuid.uuid4()
        self.client1 = self.setup_client(driver1.get())
        self.client2 = self.setup_client(driver2.get())
    ...
    def setup_client(self, driver):
        client = Client(driver)
        client.initialize()
        client.on_message.append(self.on_message)
        client.on_disconnect.append(self.on_disconnect)
        return client
    ...
    def on_message(self, message):
        if not message.is_remote:
            ...
            if message.author.id == self.client1.local_user.id:
                self.client2.send_message(message.message)
            else:
                self.client1.send_message(message.message)
            self.last_message_time = time.time()
